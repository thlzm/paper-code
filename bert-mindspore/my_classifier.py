    ...
    for data in dataset.create_dict_iterator(num_epochs=1):
        ...
        with open(args_opt.predict_result_path ,'a') as f:
            ...
            for x in logits:
                ...
                f.write((np.exp(x)/np.sum(np.exp(x)))+'\n')
...
def move_file_to_local_dir():
    ...
    obs_path = 's3://uibe-nlp-paper/data/inputs/bert-mindspore-input/data/'
...
def move_file_to_obs():
    """工作目录本地文件移动到obs"""
    local_path = os.path.join(os.getcwd(), 'data')  # /home/work/user-job-dir
    obs_path = 's3://uibe-nlp-paper/data/inputs/bert-mindspore-input/'

    # data文件夹拷到obs
    print('Copy begin')
    mox.file.copy_parallel(os.getcwd(), obs_path)
    print('Copy Success')
    print('-' * 30)
    print(local_path)
    print(os.listdir())
